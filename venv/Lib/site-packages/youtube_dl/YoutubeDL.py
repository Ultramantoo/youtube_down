...
class YoutubeDL(object):
    ...
    def extract_info(self, url, download=True, ie_key=None, extra_info={},
                     process=True, force_generic_extractor=False, num_list=None):
        '''
        Returns a list with a dictionary for each video we find.
        If 'download', also downloads the videos.
        extra_info is a dict containing the extra values to add to each result
        '''
        # print("启动：extract_info")
        # print(num_list)
        ...
        for ie in ies:
            ...
            try:
                ...
                # print(type(ie_result))
                date_pass = False
                if num_list[0] == "date":
                    # print(num_list)
                    # print('日期判断:%s' % num_list)
                    try:
                        # print(ie_result)
                        # print(int(num_list[1]))
                        uoload_dates = ie_result['upload_date']
                        # 是否下载历史
                        #[不下载]超过20分钟的
                        if int(ie_result['duration']/60) > 13:
                            print('超13分钟不下载')
                            date_pass = True
                        # print(uoload_dates)
                        # print("日期判断")
                        # print(num_list)
                        # print(num_list[3])
                        # print(int(uoload_dates))
                        # print(num_list[4])
                        if int(uoload_dates) < int(num_list[1]) and num_list[2] == "否":
                            print("到达截至日期...")
                            return False
                        # print("有值啊")
                        elif num_list[2] == "是":
                            if int(uoload_dates) < int(num_list[3]):
                                print("历史下载，后范围内日期...")
                                return False
                            elif int(uoload_dates) > int(num_list[4]):
                                print("历史下载，前范围内日期...")
                                # return
                                date_pass = True
                    except:
                ...
                if process:
                    # print("test2")
                    # 跳过条件
                    num_lists = num_list[0:5]
                    # print(num_lists)
                    if date_pass:
                        num_lists.append('pass')
                        # print(num_lists)

                        # print(num_lists)
                    # print(num_list)
                    return self.process_ie_result(ie_result, download, extra_info, num_list=num_lists)
    ...
    def process_ie_result(self, ie_result, download=True, extra_info={}, num_list=False):
        """
        ...
        if result_type == 'video':
            ...
            try:
                return self.extract_info(ie_result['url'],
                                         ...
                                         extra_info=extra_info, num_list=num_list)
            except:

                # num_lists.append('pass')
                print('下载异常跳过处理test:%s' % num_list)
                return True
            ...
            info = self.extract_info(
                ...
                extra_info=extra_info, download=False, process=False, num_list=num_list)
            ...
            return self.process_ie_result(
                new_result, download=download, extra_info=extra_info, num_list=num_list)
            ...
            for i, entry in enumerate(entries, 1):
                # print('走起 enumerate %s,->%s' % (num_list, i))
                ...
                # 增加日期跳过
                # print('获取日期：%s' % entry.get('uploader_id'))
                # info_dict.get('upload_date')
                # print('更新日期:%s'% entry['upload_date'])
                # print(num_list)
                ...
                entry_result = self.process_ie_result(entry,
                                                      download=download,
                                                      extra_info=extra, num_list=num_list)
                # 返回停止
                if not entry_result: return
            ...
            ie_result['entries'] = [
                self.process_ie_result(_fixup(r), download, extra_info, num_list=num_list)
    ...
    def download(self, url_list, num_list=None):
        ...
        for url in url_list:
            try:
                ...
                res = self.extract_info(
                    url, force_generic_extractor=self.params.get('force_generic_extractor', False), num_list=num_list)
